...
class EngineerInsights {
        ...
        metricsDiv.innerHTML = `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 11px;">
                ...
                <div style="grid-column: 1 / -1; border-top: 1px solid #00ff88; padding-top: 5px; margin-top: 5px;">
                    ...
                    Current Scenario<span class="module-tooltip" title="Current Scenario shows which specialized battle environment is currently active. Scenarios include obstacle courses, hill control maps, and specific tactical challenges designed to test different aspects of tank AI behavior and adaptation." style="cursor: help; color: #888; margin-left: 3px; font-size: 8px;">❓</span>: ${this.metrics.currentScenario || 'None'}<br>
                    Seeded Battles<span class="module-tooltip" title="Seeded Battles counts deterministic battles run with fixed random seeds. These battles can be reproduced exactly, allowing for consistent testing and comparison of different tank configurations under identical conditions." style="cursor: help; color: #888; margin-left: 3px; font-size: 8px;">❓</span>: ${this.metrics.seededBattles}<br>
                    Obstacle Interactions<span class="module-tooltip" title="Obstacle Interactions tracks how many times tanks have encountered and navigated around battlefield obstacles. This measures spatial intelligence and pathfinding capabilities in complex environments." style="cursor: help; color: #888; margin-left: 3px; font-size: 8px;">❓</span>: ${this.metrics.obstacleInteractions}<br>
                    Hill Control Attempts<span class="module-tooltip" title="Hill Control Attempts counts how many times tanks have tried to capture and hold strategic positions on hill control maps. This measures territorial control strategy and positional awareness." style="cursor: help; color: #888; margin-left: 3px; font-size: 8px;">❓</span>: ${this.metrics.hillControlAttempts}<br>
                    Reproducibility Checks<span class="module-tooltip" title="Reproducibility Checks counts verification tests to ensure seeded battles produce consistent results. This validates the deterministic nature of the simulation and helps identify any non-deterministic behavior." style="cursor: help; color: #888; margin-left: 3px; font-size: 8px;">❓</span>: ${this.metrics.reproducibilityChecks}
                </div>
    ...
    updateLogsDisplay() {
        ...
        logsDiv.innerHTML = recentLogs.map(log => {
            const colorMap = {
                ...
                // Phase 2: Enhanced Battle Scenarios
                'SEEDED_BATTLE': '#88ff00',
                'OBSTACLE_INTERACTION': '#ff8800',
                'HILL_CONTROL': '#0088ff',
                'REPRODUCIBILITY': '#ff0088'
            };
    ...
    // ======= Phase 2: Enhanced Battle Scenarios Tracking =======
    /**
     * Track seeded battle initialization
     */
    trackSeededBattle(scenarioId, seed) {
        this.metrics.seededBattles++;
        this.metrics.currentScenario = scenarioId;
        this.log('SEEDED_BATTLE', `Initialized deterministic battle: ${scenarioId} (seed: ${seed})`);
    }
    /**
     * Track obstacle interaction events
     */
    trackObstacleInteraction(tankId, interactionType, obstacleData) {
        this.metrics.obstacleInteractions++;
        this.log('OBSTACLE_INTERACTION', `Tank ${tankId} ${interactionType} obstacle at (${obstacleData.x}, ${obstacleData.y})`);
    }
    /**
     * Track hill control strategies
     */
    trackHillControlAttempt(tankId, team, distance, strategy) {
        this.metrics.hillControlAttempts++;
        this.log('HILL_CONTROL', `Tank ${tankId} (${team}) attempting hill control: ${strategy} (distance: ${distance.toFixed(1)})`);
    }
    /**
     * Validate battle reproducibility with seed
     */
    trackReproducibilityCheck(seed, battleResults) {
        this.metrics.reproducibilityChecks++;
        const resultHash = this.hashBattleResults(battleResults);
        this.log('REPRODUCIBILITY', `Seed ${seed} produced result hash: ${resultHash}`);
        return resultHash;
    }
    /**
     * Generate hash for battle results to verify reproducibility
     */
    hashBattleResults(results) {
        const hashData = {
            winner: results.winner,
            duration: Math.floor(results.duration / 100), // Round to avoid floating point issues
            redScore: results.redScore,
            blueScore: results.blueScore,
            tanks: results.tanks?.length || 0
        };
        return btoa(JSON.stringify(hashData)).substring(0, 8);
    }
    /**
    ...
    getScenarioAnalysis() {
        return {
            currentScenario: this.metrics.currentScenario,
            seededBattles: this.metrics.seededBattles,
            obstacleInteractions: this.metrics.obstacleInteractions,
            hillControlAttempts: this.metrics.hillControlAttempts,
            reproducibilityChecks: this.metrics.reproducibilityChecks,
            interactionRate: this.metrics.battlesRun > 0 ? (this.metrics.obstacleInteractions / this.metrics.battlesRun).toFixed(2) : 0,
            hillControlRate: this.metrics.battlesRun > 0 ? (this.metrics.hillControlAttempts / this.metrics.battlesRun).toFixed(2) : 0
        };
    }
